...
uint8_t my_edf_implementation() {
    ...
    for (int task_ID = 1; task_ID < NRK_MAX_TASKS; task_ID++) {
...
void _nrk_scheduler() {
    ...
    for (task_ID = 0; task_ID < NRK_MAX_TASKS; task_ID++) {
        ...
        if (nrk_task_TCB[task_ID].task_ID == -1) {     //yates nrk_task_TCB->task_ID
				        continue; //yates??? how to skip it ? 
				}
        ...
        // Note, this step changes nothing about the task's state -- just its suspend flag changes.
				nrk_task_TCB[task_ID].suspend_flag = 0 ;  //yates
        ...
        if (nrk_task_TCB[task_ID].task_ID != NRK_IDLE_TASK_ID && nrk_task_TCB[task_ID].task_state != FINISHED) {   //yates
            ...
            // Hint: next_wakeup is the number of ticks before the task will be woken up again
					   if (nrk_task_TCB[task_ID].next_wakeup > _nrk_prev_timer_val){   //yates
								nrk_task_TCB[task_ID].next_wakeup -= _nrk_prev_timer_val;   //yates
						 }
						 else if (nrk_task_TCB[task_ID].next_wakeup <= _nrk_prev_timer_val){   //yates
							  nrk_task_TCB[task_ID].next_wakeup = 0; 												  //yates
						 }
            ...
            // Hint: next_period is the number of ticks until the next period for a task starts
						if (nrk_task_TCB[task_ID].next_period > _nrk_prev_timer_val){
							...
							nrk_task_TCB[task_ID].next_period = nrk_task_TCB[task_ID].period;
							      ...
							      nrk_task_TCB[task_ID].next_period = nrk_task_TCB[task_ID].period; //yates??? what do i do here?
						}
        ...
        if (nrk_task_TCB[task_ID].task_state == SUSPENDED) {
            ...
            if (nrk_task_TCB[task_ID].next_wakeup == 0 ) {     //yates
                ...
                // For next_wakeup, think about the next time we'd want to add this task to the ready queue
							nrk_task_TCB[task_ID].cpu_remaining = nrk_task_TCB[task_ID].cpu_reserve;     //yates
							nrk_task_TCB[task_ID].task_state = READY;     															//yates
							nrk_task_TCB[task_ID].next_wakeup	= nrk_task_TCB[task_ID].next_period;     //yates
                //nrk_task_TCB[task_ID].cpu_remaining = TODO   //  to cpu_reserve
                //nrk_task_TCB[task_ID].task_state    = TODO  //to READY
                //nrk_task_TCB[task_ID].next_wakeup   = TODO   // to next_period
            
                // TODO: uncomment this line once you've implemented the rest of the for loop
                nrk_add_to_readyQ(task_ID);         //yates
        ...
        task_ID = my_edf_implementation();    // Dynamically compute task to execute
			//printf("running EDF");
#else
        task_ID = nrk_get_high_ready_task_ID(); // Pull task to execute from prio. q.
			//printf("running RM");
#endif
