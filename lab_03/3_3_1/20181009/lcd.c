...
void clearline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
	//sorting
	uint8_t xMax;
	uint8_t xMin;
...
/*void move_paddle(){
	...
	if (x<=64)  // paddle A
	{
		...
		if		(old_paddleA > y)	{updown = 1;}
		else if (old_paddleA < y)	{updown = 0;}
		else return;
			
		if (updown==1) 
		{
			for (int i = old_paddleA ; i >= y ; i--)
			{
				draw_paddleA(i);
			}
			old_paddleA = y;
	...
	{
		if		(old_paddleB > y)	{updown = 1;}
		else if (old_paddleB < y)	{updown = 0;}
		else return;
		
		if (updown==1) {
			
			for (int i = old_paddleB ; i >= y ; i--){
...
//void move_ball(1,2){// these should point to location of ballx and bally
//	;
//}
